...
public class ConsoleUi {
  ...
  Scanner mm = new Scanner(System.in, "UTF-8"); 
  ...
  /**
   * Prompts for and returns if the player is a bot or not.
   */
  public boolean pcOrNot(String playerName) {
    System.out.println("Is the player " + playerName + " a computer player? ");
    System.out.println("(Y) for yes or anykey if not ");
    String hh = mm.nextLine();
    hh = hh.strip();
    if (hh.equals("y") || hh.equals("Y")) {
      return true;
    } else {
      return false;
    }
  ...
  public void gameOver(Iterable<Player> players) {
    mm.close(); // end of game close scanner
  ...
  /**
   * Prompts the user for some input when it's Pc's turn.
   */
  public void promptForPcAction(String playerName) {
    System.out.println("It is Pc turn: " + playerName);    
    System.out.println("Press anykey and enter for pc to play: ");
    getInputChar();
  }
